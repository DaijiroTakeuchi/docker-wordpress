...
function shortcode_content($atts, $content) {
   return '<div class="shortcode_alert">' . $content . '</div>';
}
add_shortcode('my_code', 'shortcode_content');

// ショートコード ボックスを表示する
function shortcode_notes($atts, $content) {
    extract(shortcode_atts(array(
        'class' => 'normal',
        'title' => 'タイトル',
    ), $atts));
    return '<div class="shortcode_notes ' . $class . '">
        <h4 class="title">' . $title . '</h4>
        <div class="content">' . $content . '</div>
        </div>';
}
add_shortcode('my_notes', 'shortcode_notes');
